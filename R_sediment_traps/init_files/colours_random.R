...
# getPalette = colorRampPalette(c(brewer.pal(8, "Dark2"), brewer.pal(12, "Paired"), brewer.pal(8, "Set1"), brewer.pal(8, "Pastel2"), brewer.pal(8, "Accent"))) 

getPalette = colorRampPalette(c(brewer.pal(12, "Paired"), brewer.pal(8, "Set1"), brewer.pal(8, "Pastel2"), brewer.pal(8, "Accent")))

divisionList = rbind(unique(tax_table(ps)[,"subdivision"]), "Others")
divisionPalette = getPalette(length(divisionList))
names(divisionPalette) = divisionList
divisionPalette["Others"] <-"grey"
divisionPalette <- as.data.frame(divisionPalette) %>%
  rownames_to_column(var = "division") %>%
  mutate(divisionPalette = case_when(divisionPalette == "#FF9207" ~ "#345634",
                                     divisionPalette == "#A6CEE3" ~"#1F78B4",
                                     TRUE~as.character(divisionPalette))) 
